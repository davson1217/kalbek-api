...
import type { CharacterOption, ScenarioDetail, ScenarioSummary } from '../../../types';

type ScenarioFormRecord = ScenarioDetail | ScenarioSummary;

export function ScenarioForm({ action, characters, levels, method = 'post', onSuccess, scenario, statuses }: { action: string; characters: CharacterOption[]; levels: string[]; method?: 'post' | 'put'; onSuccess?: () => void; scenario?: ScenarioFormRecord; statuses: string[] }) {
    const form = useForm({
    ...
    return (
        ...
        >
            ...
            <SelectField label="Tone" help="A visual theme used by the learner app for this scenario card." value={form.data.tone} onChange={(value) => form.setData('tone', String(value))} options={['primary', 'amber', 'berry', 'sky'].map((tone) => ({ value: tone, label: tone }))} />
